...
#$func0 = "select count(received) as receive, count(shipped) as ship, count(post_tested_n20C) as tested, DATE_FORMAT(received, \"%j\") as day, DATE_FORMAT(received, \"%y-%m-%d\") as date from times_module_p where received > \"2015-09-01\" group by day order by received";
#$func1 = "select count(a.id) as num, DATE_FORMAT(received, \"%j\") as day,  date_format(received,\"%y-%m-%d\") as date from module_p a, times_module_p b where a.id=b.assoc_module and b.received>\"2015-09-01\" and a.tested_status=\"Rejected\" and b.post_tested_n20C is null group by day order by received";
...
#WEEK(DATE_SUB(received, INTERVAL 3 DAY))
#$arrIDs = array();
#for($z=0;$z<count($arr[0]);$z++){
#	#echo $arr[0][$z];
#	$funcIDs = "select a.id from times_module_p b, module_p a where DATE_FORMAT(received,\"%y-%m-%d\") = \"".$a#rr[0][$z]."\" and a.id=b.assoc_module";
#	#echo $funcIDs."<br>";
...
#// Setup the titles
$graph->title->Set('Module Assembly and Testing by Batch');
