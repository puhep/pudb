...
if(!is_null($file1)){
	for($loop=0;$loop<$datacount1;$loop++){
		...
		$active=$doc1->DATA_SET->DATA[$loop]->ACTV_CURRENT_AMP;
		$guard=$doc1->DATA_SET->DATA[$loop]->GRD_CURRENT_AMP;
		$total=$doc1->DATA_SET->DATA[$loop]->TOT_CURRENT_AMP;
		settype($active,"float");
		settype($guard,"float");
		settype($total,"float");
		$arr1[1][$loop]=abs($active)+abs($guard)-abs($total);
		#$arr1[1][$loop]=$doc1->DATA_SET->DATA[$loop]->ACTV_CURRENT_AMP;
...
if(!is_null($file1)){
	for($loop=0;$loop<$datacount1;$loop++){
		...
		$arr2[0][$loop]=$doc1->DATA_SET->DATA[$loop]->VOLTAGE_VOLT;
		
		settype($arr2[0][$loop],"float");

		$arr2[1][$loop]=$doc1->DATA_SET->DATA[$loop]->ACTV_CURRENT_AMP;
		if($arr2[1][$loop] == "NaN"){
			$arr2[1][$loop] = 1E-10;
		}
...
if(!is_null($file1)){
	for($loop=0;$loop<$datacount1;$loop++){
		...
		$arr3[0][$loop]=$doc1->DATA_SET->DATA[$loop]->VOLTAGE_VOLT;
		
		settype($arr3[0][$loop],"float");

		$active=$doc1->DATA_SET->DATA[$loop]->ACTV_CURRENT_AMP;
		$guard=$doc1->DATA_SET->DATA[$loop]->GUARD_CURRENT_AMP;
		settype($active,"float");
		settype($guard,"float");
		$arr3[1][$loop]=abs($active) + abs($guard);
		if($arr3[1][$loop] == "NaN"){
			$arr3[1][$loop] = 1E-10;
		}
...
if(!is_null($file1)){
	for($loop=0;$loop<$datacount1;$loop++){
		...
		$arr4[0][$loop]=$doc1->DATA_SET->DATA[$loop]->VOLTAGE_VOLT;
		
		settype($arr4[0][$loop],"float");

		$arr4[1][$loop]=$doc1->DATA_SET->DATA[$loop]->TOT_CURRENT_AMP;
		if($arr4[1][$loop] == "NaN"){
			$arr4[1][$loop] = 1E-10;
		}
